...
export default function QuestionDetail({ question }) {
  ...
  // ---------------------------------------------
  // Parse backend field: questionDetail
  // ---------------------------------------------
  const detail = question.questionDetail || "";

  const parsed = {
    description: detail.split("Tips:")[0]?.trim() || "",
    tips:
      /Tips:\s*([\s\S]*)/i.exec(detail)?.[1]?.split("Difficulty:")[0]?.trim() ||
      "",
    difficulty: /Difficulty:\s*(Easy|Medium|Hard)/i.exec(detail)?.[1] || "",
  };
  ...
  return (
    <div className="question-detail">
      ...
      <div className="question-card p-3 shadow-sm rounded bg-light">
        <p>
          <strong>Company:</strong> {question.company || "Unknown"}
        </p>
        <p>
          <strong>Role:</strong> {question.role || "Not specified"}
        </p>
        <p>
          <strong>Difficulty:</strong> {parsed.difficulty || "Not mentioned"}
        </p>
        ...
        <div className="question-body mt-3">
          ...
          <p className="mt-2 text-muted">
            {parsed.description || "No detailed description available."}
          </p>
        </div>

        <div className="tips-section mt-3">
          <strong>Tips / Advice:</strong>
          <p>{parsed.tips || "No tips shared for this question."}</p>
        </div>
...
QuestionDetail.propTypes = {
  question: PropTypes.shape({
    ...
    questionDetail: PropTypes.string, // backend field
