...
router.post("/", authMiddleware, async (req, res) => {
    try {
        ...
        if (!body.company || !body.questionTitle) {
            return res.status(400).json({ error: "company and questionTitle are required" });
        }
...
router.get("/", async (req, res) => {
    try {
        ...
        if (isNaN(pageNumber) || pageNumber < 1) return res.status(400).json({ error: "Invalid page number" });
        if (isNaN(limitNumber) || limitNumber < 1) return res.status(400).json({ error: "Invalid limit" });
        ...
        const questionsList = await db.collection("questions")
            .find(query)
            .sort(sortOptions)
            .skip(skip)
            .limit(limitNumber)
        ...
        const userIds = questionsList
            .map(q => {
                try { return new ObjectId(q.userId); } catch (e) { return null; }
            })
            .filter(id => id !== null);
...
router.put("/:id", authMiddleware, async (req, res) => {
    try {
        ...
        const allowedFields = ["company", "role", "questionTitle", "questionDetail"];
        ...
        for (const key of allowedFields) {
            if (updates[key] !== undefined) {
                allowedUpdates[key] = typeof updates[key] === 'string' ? updates[key].trim() : updates[key];
            }
        }

        if (Object.keys(allowedUpdates).length === 0) return res.status(400).json({ error: "No valid fields" });

        allowedUpdates.updatedAt = Date.now();
        await db.collection("questions").updateOne({ _id: id }, { $set: allowedUpdates });
